...
#adapted to be-able to go to request one ahead of current index before request is made
def send_car(car, index):
    ...
    if car["next_available_time"] > time_stamps[index]:
        ...
        car["last_drop_off"] = dest_nodes[request] - 1
        if (car["next_available_time"] + getting_to_source) < time_stamps[index]:
            car["next_available_time"] = time_stamps[index] + getting_to_dest
            wait_times.append(0)
        else:
            # next available time would be the request time plus the getting to pick and getting to drop off
            wait_times.append((car["next_available_time"] + getting_to_source) - time_stamps[index])
            car["next_available_time"] += (getting_to_source + getting_to_dest)
...
print("Distance from 2-25", nx.dijkstra_path_length(graph, 1, 24))
